...
export default function DashboardContent() {
    ...
    async function fetchData() {
      try {
        ...
        const confirmedRevenue = deliveredOrdersRes.data
          ? deliveredOrdersRes.data.reduce((sum: number, order: any) => sum + (order.total_price || 0), 0)
          : 0
          
        const pendingRevenue = pendingOrdersRes.data
          ? pendingOrdersRes.data.reduce((sum: number, order: any) => sum + (order.total_price || 0), 0)
          : 0
        ...
        const orderUserIds = recentOrders.map((order: any) => order.user_id).filter(Boolean)
        ...
        orderCustomers?.forEach((customer: any) => {
  ...
  const { productCount, orderCount, customerCount, confirmedRevenue, pendingRevenue, recentActivities, salesData } = data
        ...
        <div>
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-7">
            <div className="lg:col-span-4">
              <SalesChart data={salesData.map((d: any) => ({ name: new Date(d.day).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), total: d.total }))} />
            </div>
